...
就是在缓存失效时，如果有多个请求过来，那么只能有一个请求可以去进行缓存操作，解决缓存击穿的问题。关于缓存击穿的问题，如果大家不记得了，可以去 Redis 系列的文章中查看哦。**Redis进阶：缓存穿透、击穿与雪崩**[]()
...
默认值是 off ，过请求头中的 HTTP 缓存相关字段来做为缓存的更新依据，需要我们 PHP 代码中添加响应头及处理。
...
如果只指定缓存时间 ，then only 200, 301, and 302 responses are cached，此外，可以指定 any 参数来缓存任何响应
...
代理 很简单吧，就是打开了一个随机数。另外我们还根据不同的 GET 参数 code ，返回不同的响应状态码，比如我们要返回 500 状态码，就直接加上一个 `code-=500` 这样的 GET 参数就好了。
...
关于缓存文件的查看，大家直接 Nginx 程序运行目录下的 proxycache 目录下就可以看到了，内容和 FastCGI 生成的缓存文件内容是一样的。
