...
关于 TIME_WAIT ，其实是为了能够正确、自然地进行 TCP 四次挥手而预留的等待时间，更具体的内容，大家可以再自行查阅相关的知识点。同时，net.ipv4.tcp_tw_reuse 也要慎用，当客户端与服务端主机时间不同步时，客户端的发送的消息有可能会被直接拒绝掉。正式环境使用时，建议：别动！保持默认就好。
