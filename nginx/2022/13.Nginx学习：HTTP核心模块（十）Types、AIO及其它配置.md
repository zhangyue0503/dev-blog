...
AIO 是啥东西？它是 Linux 和 FreeBSD 这类操作系统下的一种异步 IO 功能。它的作用是可以允许进程发起很多 IO 操作，而不用阻塞或等待任何操作完成。稍后在收到 IO 操作完成的通知时，进程就可以检索 IO 操作的结果。注意，它和 epoll 不是一个东西，而且 Nginx 中可以组合 aio 和 epoll 来一起使用。AIO 相关的配置项都可以用于 http、server、location 各个模块中。
