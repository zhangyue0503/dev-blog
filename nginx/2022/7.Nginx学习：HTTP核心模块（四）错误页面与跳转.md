...
这个指令应该是比较常见了吧，为指令错误定义显示的URI。

```shell
error_page code ... [=[response]] uri;
```

当前配置级别没有 error_page 指令时，将从上层配置继承， URI 可以包含变量。一般对于错误页面，我们想要友好提示，都会准备一些页面，比如说网上有包含很多设计元素的 404 页面模板。code 可以设置多个，=response 可以修改返回的响应码，最后的 uri 就是我们要最终显示哪个 uri 页面。先来看看默认的 Nginx 错误页面的配置是啥样的吧。
...
看出来了吧，默认情况下 404 确实是有页面的，而 500 页面也走的是 html 目录下的那个  50x.html 。现在我们来配置几个页面。
...
我们将全局的 404 报错页面修改为一个不存在的文件，那么最后返回的错误页面又会变成最前面的的我们看到的那个 Nginx 默认的 404 页面。

![img/errorpage1.png](/Users/zhangyue/MyDoc/博客文章/dev-blog/nginx/2022/img/errorpage1.png)

如果不修改全局的，只是修改 /errtest1/ 的配置，那么最终会显示外层的 404 错误页面，这个大家可以自己尝试一下哦。
...
啥意思呢？首先，默认值是 off ，在默认的情况下，Nginx 会以请求头中的 Host 字段为基础，其实就是我们访问 URL 的 HOST 部分啦。这样在跳转链接的时候，如果 `absolute_redirect` 是 `on`，并且  `return` 中也中不绝对路径的话，就会自动将 HOST 添加上，组成完整的 URL 。
