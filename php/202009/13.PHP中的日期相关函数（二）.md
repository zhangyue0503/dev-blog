...
上回文章中我们介绍了三个时间日期相关的对象，不过它们的出镜频率并不是特别地高。今天学习的对象虽说可能不少人使用过，但是它的出镜频率也是非常低的。但，酒香不怕巷子深，好东西还是值得我们去深入的学习研究的，当然更好的情况是可以在面对不同的业务场景时灵活地使用这些对象方法才是我们学习的根本目的。
...
diff() 方法就是返回两个日期之间的差值，相信这个方法不少人使用过，相对于其它方法来说，它的出镜率就非常高了。%R 返回的是符号位，如果是负号就是比指定的日期少了多少时间间隔。
