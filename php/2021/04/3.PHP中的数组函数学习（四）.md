...
提到遍历数组很多人第一个想到的就是 array_map() ，这个函数我们会讲，不过首先我们来看的是另外一个 array_walk() 函数。
