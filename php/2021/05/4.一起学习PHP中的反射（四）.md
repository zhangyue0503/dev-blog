...
isDisabled() 这个方法是 ReflectionMethod 中没有，这是 ReflectionFunction 中所特有的一个方法。它的作用是判断函数是否可用，其实就是判断这个函数是否在 php.ini 中被禁用了，也就是在 php.ini 中的 disable_functions 中是否添加了这个函数。我们在本机的测试环境中增加了 dl 方法到 disable_functions 中，在这里使用 isDisabled() 就会返回 true 。
...
isVariadic() 用于判断函数的参数是否的可变长度的参数。isGenerator() 则用于判断函数是否为生成器函数。这两个也不用多做解释了，注意查看函数原型之间的区别。
...
还记得生成器是个什么东西吗？不记得的小伙伴可以去 [学习PHP生成器的使用]() 这里看下哦，或者在公众号回复这个文章的标题也可以看到这篇文章。通过反射，我们也可以操作一个生成器函数，是不是很高大上。
