...
有两个实例化的方法，不过它们的功能是一样的，只是参数不一样，newInstance() 方法会将构造参数一个一个地传递进去，而 newInstanceArgs() 则是可以用一个数组来传递实例化对象的构造参数。这个其实非常像 call_user_func() 和 call_user_func_array() ，还记得这两个家伙吗？不记得的小伙伴可以传送到 [PHP中的函数相关处理方法学习]() 这里复习一下。下面我们要学习到的反射方法类中也有类似的方法函数哦。
