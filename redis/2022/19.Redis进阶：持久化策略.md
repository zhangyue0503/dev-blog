...
看到这个注释就好理解了吧，在多长时间内，执行了多少命令，就执行一次 BGSAVE 保存 RDB 文件。注意，这个执行的命令一定是改动了数据了命令，比如 SET ，而 GET 这类的命令是不算在内的。为啥呢？查询类的命令没啥可保存的必要呀，我们要保存的是存在的数据以及被改动的数据。
...
很明显，默认情况下是每秒写入，always 是一直写入，也就是只要有一个修改类的命令出现，马上就进行写入，一有磁盘操作，大家就能想到，它多少会拖慢速度，但很安全，因为数据基本不会丢。最后一个是不写入，相当于关闭 AOF 功能了。

不用多说了吧，always 会很耗性能。一般走默认的就好了，但是，注意，这秒内如果执行了命令，但 AOF 还没有自动写入，这时的数据也会丢失的。

### AOF 重写工作原理

和 RDB 一样，好像上面 RDB 的写入过程也没细说，那就一起说吧。它们俩都是复用的写时复制这种技术。都是 fork 子进程，将新的 AOF 内容写入临时文件。在写入时，父进程对新来的命令写入到缓存。当重写工作完成后，临时文件替换成正式文件，子进程向父进程发送信号，然后父进程将缓存中的数据写入到新的 AOF 文件。
