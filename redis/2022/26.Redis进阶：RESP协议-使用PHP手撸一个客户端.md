...
应用软件和数据库或者说和其它外部程序是怎么连接的呢？大部分同学应该都知道，基本都是通过 TCP 建立连接来进行通信的，不过也有像 ES 这种直接通过 HTTP 发请求来连接的。Redis 和大部分的数据库软件都是类似的，通过的是 TCP 连接的。
...
还记得我们在 **Redis进阶：持久化策略**[]() 中看到过的那个 aof 文件中的内容吗？如果不记得了也没关系，咱们粘过来。
...
是不是有那么一点 Redis 扩展的味道了。我们这只是非常简单的封装，如果是 Hash、Sorted Set 之类的，这样直接返回一个数组肯定还是不行的。就像文章开始的时候我们说过的，如果真的要写一套完整的客户端，需要考虑的内容远比我们现在这样简单的写一个来的要多。不过总体来说，大概的功能和协议的规则相信各位也已经清楚了。
...
有兴趣的小伙伴可以再深入地去看一下 phpredis 这个 Composer 扩展包，它是完全 PHP 写的 Redis 扩展，不是 Predis 那样通过原生的 C 语言编译的。
