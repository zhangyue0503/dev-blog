...
今天我们学习的内容是一些普通的客户端操作，这些内容有的可能你天天都会用到，而有的可能原来根本都没听说过。好吧，说得就是我，没有系统的学习之前真的很多命令都不清楚，甚至完全就是不知道的状态。当然，无知者无畏，这个无畏其实是要面对学习无所畏惧。
...
这个命令就是用来测试服务器连通情况的。比如直接 PING 一下，返回一个 PONG ，看来 Redis 的作者也是个乒乓迷啊。如果给一个参数的话，则是原样打印这个字符内容。
...
CLIENT 是一套客户端命令，也就是说它是由多二级命令组成的，具体的内容包括下面这些。
...
这些信息是一行一行的键值对，其中就能发现 id 、name 这些上面已经学习过的内容，其它比较有用的还有 addr、db、cmd 等信息，分别代表的是 连接地址+端口、当前正在操作的库、上一条执行过的命令等。剩下的字段大家可以去官方文档查阅一下是什么意思哦。
