...
在 XS 中，每个字段都是使用一个区段配置来表示的。其实就是我们一看到的中括号那个，中括号内容的是字段名，下面就是字段的选项。从一个字段到另一个字段之间的区域可以为上一个字面定义多个选项。
...
- numeric 数值型，包含整型和浮点型都是什么 numberic ，仅当字段需要用于排序或者区间检索时才设为这个类型，否则直接使用 string 类型来表示数字就行了。
...
字段类型就是这几种，比起数据库来说还是少了很多了吧。比如我们的操作是以检索数据为主，因此，大部分情况下其实都使用的就是普通的 string 类型就可以了。numberic 和 date 类型通常是有特殊的需要，比如说排序或者区间搜索时会用到，而且这两种类型默认是不会分词的，也就是和 MySQL 中的字段是一样的功能，要索引也是整个字段的内容全部当成一个完整的值来进行索引。

而另外两个 title 和 body ，其实底层都是 string ，只是它们自己默认带了一些其它配置，我们马上就来说。

### index 索引方式

索引方式和搜索方式有关。啥意思呢？在 XS 中，索引有 2 种探索方式。
...
- title 类型，默认是 both ，可能通过 `title:xxx` 来指定查询，也可以通过 xxx 这样和 body 一起混合查询。
...
- split(arg) 表示根据参数分割内容，默认参数是空格，也就是默认使用空格来分词，如果以 / 开关并以 / 结尾，那么就会调用 PHP 的 preg_split() 函数来进行正则分词
...
布尔索引是表示当前字段不参与权重排名计算的，关于权重排名计算的内容我们后面也会详细说，这里也是先了解一下就好了。默认情况下所有的自定义分词器都是布尔索引的。因此，当使用自定义分词器，却又想让当前字段参与权重计算的话，就要将这个选项设置为 yes 。
...
为什么少了一些字段呢？这个具体的设计问题，我们放到最后再说，这一个索引配置文件中，我们主要是用来测试上面学习到的那一堆配置选项用得。具体的内容在注释中都写得很清楚了。
...
官方自带的这个工具，不仅可以导入 MySQL ，还可以像之前一样导入 CSV ，如果指定了文件就可以直接从 CSV 文件中批量读取导入数据。另外，它也可以直接进入 JSON、SQLite 数据。具体用法可以不加参数运行 `/util/Indexer.php ` 来查看帮助信息。
...
默认情况下，或者你删掉上面配置中的 `index = mixed`，使用 `subject:算法` 是可以搜索到东西的，但是因为我们设置成只能通过混合区检索了，那么这样指定字段名的搜索就无效啦。  

```shell
>  php vendor/hightman/xunsearch/util/Quest.php --show-query ./config/5-zyarticle-test1.ini "subject:算法"
--------------------
...
```shell
> php vendor/hightman/xunsearch/util/Quest.php --show-query ./config/5-zyarticle-test1.ini "信管1.15"
--------------------
解析后的 QUERY 语句：Query((信管@1 AND 1.15@2))
--------------------
在 339 条数据中，大约有 1 条包含 信管1.15 ，第 1-1 条，用时：0.0246 秒。

1. 【信管1.15】安全（二）加解密技术 #843# [100%,12.87]
安全（二）加解密技术上一篇课程中我们了解到的是很多关于信息系统安全... 
...
```shell
> php vendor/hightman/xunsearch/util/Quest.php --show-query ./config/5-zyarticle-test1.ini "20221114"
--------------------
解析后的 QUERY 语句：Query(20221114@1)
--------------------
在 339 条数据中，大约有 0 条包含 20221114 ，第 0-0 条，用时：0.0119 秒。
> php vendor/hightman/xunsearch/util/Quest.php --show-query ./config/5-zyarticle-test1.ini "pub_time:20221114"
--------------------
解析后的 QUERY 语句：Query(0 * F20221114)
--------------------
在 339 条数据中，大约有 0 条包含 pub_time:20221114 ，第 0-0 条，用时：0.0121 秒。
```
...
```shell
> php vendor/hightman/xunsearch/util/Quest.php --show-query ./config/zyarticle.ini 'content:电路' 
--------------------
...
```shell
> php vendor/hightman/xunsearch/util/Quest.php --show-query ./config/zyarticle.ini '电路'         
--------------------
...
而且，各位在使用 Baidu 或者 Google 时也会发现，在列表上的内容，也就是返回标题和内容中包含关键词的一部分。和我们 XS 返回的内容是完全一样的。如果确实需要完整的内容数据，那么就使用主键 ID 去数据库查询吧，主键聚集索引在数据中的查询速度是非常快的。另外还有一种，就是本身 body 类型的字段不会有太多的内容，比如电商中的端口详情，可能图片居多，文字较少，这时是可以考虑直接 `cutlen=0` 的。
...
```shell
> php vendor/hightman/xunsearch/util/Quest.php --show-query ./config/5-zyarticle-test1.ini 'category_name:项目'     
--------------------
解析后的 QUERY 语句：Query(0 * D项目)
--------------------
在 342 条数据中，大约有 0 条包含 category_name:项目 ，第 0-0 条，用时：0.0080 秒。
```

针对这个字段，如果只搜索项目这个词，是搜不到东西的。因为对于这个字段来说，我们是将它的值完整地作为一个索引词内容的。因此，我们需要完整的分类名称。

```shell
php vendor/hightman/xunsearch/util/Quest.php --show-query ./config/5-zyarticle-test1.ini 'category_name:项目产品' 
--------------------
解析后的 QUERY 语句：Query(0 * D项目产品)
--------------------
在 342 条数据中，大约有 48 条包含 category_name:项目产品 ，第 1-10 条，用时：0.0107 秒。

1. 【敏捷0】敏捷项目管理-为什么从敏捷开始？为什么从PMI-ACP开始？ #234# [100%,0.00]
【敏捷0】敏捷项目管理-为什么从敏捷开始？为什么从PMI-ACP开始？作为敏捷... 
Category_name:项目产品  Tags:敏捷,ACP  Pub_time:20220723  
……………………
```
