...
为什么又说 临键锁 是默认的行锁呢？其实在默认情况下，行锁 就是 临键锁 ，它会锁自己以及附近的数据，但是，如果是主键或者唯一索引，会退化成 记录锁 ，也就是我们习惯说的那个 “行锁” ，而在大部分情况下，普通的间隙空值操作也会退化为 间隙锁 ，只有在一些条件下才会产生 临键锁 。因此，间隙 和 记录 这两种锁其实都是 临键锁 的退化版本，或者说是简易版本。注意，这个退货仅限于主键是由一个列组成的，如果是多个列组成的，则不会发生退化。
